	...
	if (localPlayer && interfaces::engine->IsInGame())
	{
		for (int i = 0; i < interfaces::glow->glowObjects.size; i++)
		{
			...
			case CClientClass::CCSPlayer:
				...
				{
					glowObject.SetColor(0.f, 1.f, 0.f);
				}
				else
				{
...
void vis::BoxESP()
{
	...
	for (int i = 1; i < 64; i++)
	{
		...
		CVector vecOrigin = *(CVector*)(player + hazedumper::netvars::m_vecOrigin) - CVector{0.f, 0.f, 11.f};
		auto boneMatrix = *(uintptr_t*)(player + hazedumper::netvars::m_dwBoneMatrix);
		auto headPos = CVector{
			*(float*)(boneMatrix + 0x30 * 8 + 0x0C),
			*(float*)(boneMatrix + 0x30 * 8 + 0x1C),
			*(float*)(boneMatrix + 0x30 * 8 + 0x2C) + 9.f
		};
...
CVector readBone(int i, CEntity* entity)
{
	auto boneMatrix = *(uintptr_t*)(entity + hazedumper::netvars::m_dwBoneMatrix);
	return CVector{
			*(float*)(boneMatrix + 0x30 * i + 0x0C),
			*(float*)(boneMatrix + 0x30 * i + 0x1C),
			*(float*)(boneMatrix + 0x30 * i + 0x2C)
	};
}
...
void vis::SkeletonESP()
{
	...
	std::cout << "Team: " << localPlayer->Team() << std::endl;
	...
	for (int i = 1; i < 64; i++)
	{
		...
		for (int i = 0; i < bones.size(); i++)
		{
			...
			CVector bone1 = readBone(bones[i].first, player);
			CVector bone2 = readBone(bones[i].second, player);

			if (interfaces::debugOverlay->ScreenPosition(bone1, f))
				goto jump;
			if (interfaces::debugOverlay->ScreenPosition(bone2, s))
				goto jump;
...
{
	*(int*)(pWeapon + hazedumper::netvars::m_iItemIDHigh) = -1;
	*(int*)(pWeapon + hazedumper::netvars::m_nFallbackPaintKit) = skin;
	*(float*)(pWeapon + hazedumper::netvars::m_flFallbackWear) = 0.1f;
}
...
{	
	...
	if (localPlayer) { 
		for (int i = 0; i < 8; i++) 
		{
			DWORD* cWep = (DWORD*)(localPlayer + hazedumper::netvars::m_hMyWeapons);
...
void vis::changeFOV(int FOV)
{
	...
	if (localPlayer)
	{
		*(int*)(localPlayer + hazedumper::netvars::m_iDefaultFOV) = FOV;
	}
